...
const { Iliriklagu, Iquotemaker, IrandomNimek, Ifb, Isleep, IjadwalTv, ss } = require('./lib/functions')
const { Ihelp, Idonate, IlistChannel } = require('./lib/help')
...
module.exports = msgHandler = async (client, message) => {
    try {
        ...
        const mess = {
            ...
            error: {
                St: '[❗] Kirim gambar dengan caption *Isticker* atau tag gambar yang sudah dikirim',
        ...
        case 'Insfwmenu':
            if (IisNsfw) return
            client.reply(from, '1. IrandomHentai\n2. IrandomNsfwNeko', id)
            break
        case 'Iigstalk':
        ...
        case 'Irandomhentai':
            if (isGroupMsg) {
                if (IisNsfw) return client.reply(from, 'Command/Perintah NSFW belum di aktifkan di group ini!', id)
